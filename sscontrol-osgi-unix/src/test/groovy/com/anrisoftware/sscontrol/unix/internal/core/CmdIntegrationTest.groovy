...
import com.anrisoftware.sscontrol.profile.internal.ProfilePropertiesServiceImpl
import com.anrisoftware.sscontrol.profile.internal.ProfileServiceImpl
...
class CmdIntegrationTest {
    ...
    void "run command"() {
        ...
        Profile profile = Eval.me('profile', profileService.create(), '''
...
''') as Profile
    ...
    void "run command ssh"() {
        ...
        def properties = cmdProperties.get()
        properties.put "${command}_command", echo
        ProfileImpl profile = Eval.me('profile', profileService.create(), '''
profile 'Ubuntu 12.04' with {
    hostname
}
profile
...
''') as Profile
        ...
        testCases.eachWithIndex { Map test, int k ->
            ...
            assertStringContent fileToString(new File(dir, "${command}.out")), resourceToString(CmdIntegrationTest.class.getResource(testExpected.output as String))
        }
    }
    ...
    static void createServices() {
        ...
        context.registerInjectActivateService(new TypedAllPropertiesService(), null)
        ...
        context.registerInjectActivateService(new ProfilePropertiesServiceImpl(), null)
        this.profileService = context.registerInjectActivateService(new ProfileServiceImpl(), null)
